...
@Builder
@Schema(description = "Hasar raporu oluşturma isteği")
public record CreateDamageReportRequest(
    @NotNull(message = "Araç ID gereklidir")
    @Schema(description = "Araç ID", example = "1")
    Long vehicleId,

    @Schema(description = "Kiralama ID (opsiyonel)", example = "1")
    Long rentalId,

    @NotNull(message = "Rapor tarihi gereklidir")
    @Schema(description = "Hasar rapor tarihi", example = "2026-01-26")
    ...
    @Size(max = 100)
    @Schema(description = "Raporu oluşturan kişi", example = "Ahmet Yılmaz")
    String reportedBy
) {}
